...
class EDAService:
    ...
    def missing_value_analysis(df):
        ...
        quality_score = round(
            (
                1 -
                (missing_count.sum() / (df.shape[0] * df.shape[1]))
            ) * 100,
            2
        )
    ...
    def duplicate_analysis(df):
        ...
        return {
            ...
            "sample_duplicates": duplicate_samples.to_dict(
                orient="records"
            )
    ...
    def column_classification(df):
        ...
        for column in df.columns:
            ...
            cardinality = (
                "High"
                if unique_values > total_values * 0.20
                else "Low"
            )
    ...
    def business_insights(df):
        ...
        if total_missing == 0:
            insights.append(
                "Excellent data quality. No missing values were detected."
            )
        else:
            insights.append(
        ...
        recommendations.append(
            "Perform feature engineering before model training."
        )
